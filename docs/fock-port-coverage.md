...
`cosmic_fock --help` lists all options. Numerical controls include `--basis`, `--step`, `--frames`, `--times` (comma-separated output times including both endpoints), and `--steps` (one maximum internal step per output interval), and NM error tolerances `--rtol` / `--atol` (defaults 1e-9 / 1e-11). Physics options include `--hbar`, `--mu`, `--beta3`, `--beta4`, `--lambda`, `--hubble`, `--volume`, `--omega`, `--initial`, and `--final`. `--noise` supplies one standard normal value per SSE integration step, including inactive intervals; `--bath-noise` supplies four real/imaginary pairs. Both are exported for replay. RNG streams themselves need not agree between platforms or languages. Unused or missing supplied SSE samples are errors.

The default Fock basis of 24 is a demonstration setting, **not** a converged replacement for the original basis-400 calculations. The obsolete explicit high-power SSE algorithms have been replaced by exact measurement substeps. Splitting error still needs refinement. Refine the timestep and basis together; normalization alone does not demonstrate accuracy.
...
| `IndividualTrajectories_Z2.m` | `--workflow sse/lindblad --model x2`, then renderer | X² exact measurement, density and media |
| `IndividualTrajectories_X3.m` | `--workflow sse/lindblad --model x3`, then renderer | X³ exact measurement, density and media |
...
| `SchrodingerSingleTrajectory_ExpStep_1000.m` | `--workflow closed` | Fourth-order unitary composition; unnormalized projected output; wrapper |
| `SSEDynamics_X_Sparse.m` | `--workflow sse --model x` | Supplied-noise MATLAB comparison and ensemble tests |
| `SSEDynamics_X2_Sparse.m` | `--workflow sse --model x2` | Exact measurement supplied-noise comparison, switch and refinement |
| `SSEDynamics_X3_Sparse.m` | `--workflow sse --model x3` | Exact measurement supplied-noise comparison, variable steps and refinement |
...
| `NonMarkovianScript.m` | `--workflow nm-sse/nm-density`, then renderer | Both numerical paths and NM media |
| Extracted `NMQSD_SingleTrajectory_Hybrid.m` | `--workflow nm-sse` | Standalone MATLAB helper versus common solver |
| Extracted `NMQSD_MasterEquation_lowrank.m` | `--workflow nm-density` | Independent full-history quadrature and nonuniform-grid comparisons |
...
| `GaussianMeasurementStep.m` | Exact Gaussian instrument | Born populations, exact dephasing, strong-measurement tests |
| `AdaptiveMemoryStep.m` | Dormand-Prince 5(4) | Independent DOP853 reference, tolerance refinement |
...
- SSE evolves in 2b and normalizes projected b-dimensional snapshots. All powers use Gaussian Born-mixture measurement and unitary spectral Strang splitting. At zero coupling, the same step is unitary. No finite-step Euler/SRK alternative remains.
- Closed evolution uses fourth-order symmetric unitary composition in 3b and deliberately retains the norm lost in projection. Adiabatic states use 2b and normalize the projection.
...
- NM trajectory uses the retained hybrid scheme: big-basis unitary propagation before -1, then adaptive Dormand-Prince integration of the small-basis augmented memory equations. NM density implementations retain their own equation active from the initial time; they are not asserted to be an exact ensemble equivalent of the trajectory approximation.
- The rank-four bath has covariance `3*(9*cos(omega*dN)+cos(3*omega*dN))/(128*H^3*omega^3)`. Production memory uses rank-factorized continuous integrals initialized to zero, with adaptive error control. Direct full-history quadrature exists only in tests. In the standalone MATLAB hybrid helper, an inserted switch sample is interpolated from the supplied discrete bath/noise data.

## Recorded corrections

1. The earlier X³ normalization repair left Euler timestep sensitivity. Euler/SRK have now been removed and replaced by exact Gaussian measurement substeps; see [solver choices](solver-choices.md).
...
4. NM density memory sums used a single constant timestep. Adaptive augmented equations now integrate the memory; direct full-history Simpson quadrature checks the contraction independently.
5. Cached Hermitian spectral propagation removes the exponential-action dependency and the temporary Taylor implementation. Numerical failures raise errors rather than returning partial success.
