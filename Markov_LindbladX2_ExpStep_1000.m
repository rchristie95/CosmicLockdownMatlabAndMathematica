function [plotSpan,RhoStore] = Markov_LindbladX2_ExpStep_1000(TSpan,bSize,hbar,mu,beta3,beta4,lambda,Hb,displayFlag,maxStep,volume)
...
c=struct('workflow','lindblad','model','x2','basis',bSize,'hbar',hbar,'mu',mu,'beta3',beta3,'beta4',beta4,'H',Hb,'lambda',lambda,...
    'initial',TSpan(1),'final',TSpan(end),'frames',1001,'step',maxStep,'volume',volume);
r=FockWorkflow(c); plotSpan=r.times; RhoStore=r.rho;
if displayFlag, fprintf('X2 GKLS: %d snapshots, final trace %.12g\n',numel(plotSpan),real(trace(RhoStore(:,:,end)))); end
end
