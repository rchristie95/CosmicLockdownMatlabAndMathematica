...
plotSpanInst=linspace(N0,Nend,1001);
...
for k = 1:1001
    ...
    WigImages(:,:,k)= PsiWigner( PsiInstPos(:,k), x, p, hbar)/norm( PsiInstPos(:,k));
...
% fig = figure('Position', [100 100 800 600]);  % wider figure for visibility
% ax = axes(fig); hold(ax,'on');
% hB = pcolor(ax, x, p, WigImages(:,:,501));
% shading('interp')
% % set(hB,'Interpolation','bilinear');   % or 'bicubic'
% colormap(ax,'parula');
% contour(ax, xMesh1, pMesh1, Hgrid_at(targets(2)), levels, 'LineColor', 'k');
% xlim(ax,[-Xview Xview]); ylim(ax,[-Xview Xview]); axis(ax,'square'); box(ax,'on'); caxis(ax,cax);
% expLine = plot(expXInst(1:501), expPInst(1:501), 'w-', 'LineWidth', 1); % White line with specified width
% xticks(linspace(-Xview,Xview,7))
% 
% colorbar(ax,'FontSize',18);
% text(ax, 0.98, 0.98, fmtText(targets(2), CtInst(501)), ...
%     'Units','normalized','HorizontalAlignment','right','VerticalAlignment','top', ...
%     'Interpreter','latex','FontSize',24,'FontWeight','bold','Color','w');
% set(ax, 'FontSize', 20, 'LineWidth', 1.5, 'Box', 'on', 'TickLabelInterpreter', 'latex');
% xlabel(ax,'$\phi$','Interpreter','latex','FontSize',25);
...
% fig = figure('Position', [100 100 800 600]);  % wider figure for visibility
% ax = axes(fig); hold(ax,'on');
% hC = pcolor(ax, x, p, WigImages(:,:,end));
% shading('interp')
...
RhoLind200 = RhoLind(1:200, 1:200, :);

% [plotSpanLind, RhoLind] = Markov_2Lindblads_Mixed_ode15sP_Exp( TSpan,bSize, hbar, mu,beta3, beta4, lambda, Hb,true);
...
% expXLind=expXHam;
% expPLind=expPHam;
filenameWorkspace = sprintf('Workspace_XandP_Individuals_lambda%.2g_Hubble_%.2g.mat', lambda, Hb);
...
% eta=(randn(Nsteps+1,1));1`
tic
% [~,PsiSSE, RhoSSE] = SSEDynamics_XandP_Sparse_Wikt(   bSize, hbar, mu, beta3, beta4, lambda, Hb,plotSpanSSE,dN_plot);
[~,PsiSSE, RhoSSE] = SSEDynamics_XandP_Sparse(   bSize, hbar, mu, beta3, beta4, lambda, Hb,plotSpanSSE,dN_plot);
...
title( sprintf(...
    'SSE $(\\hat L_{\\phi})$ and $(\\hat L_{\\pi_{\\phi}})$, $\\lambda = %.2f$, $\\tilde \\mu = %.2f$', ...
    lambda, mu/Hb), ...
    'FontSize', 20, 'Interpreter', 'latex' );
...
% Save video
SSEVideoPath = sprintf('Wigner_XandP_SSEVideo_lambda_%.2g_Hubble_%.2g.avi', lambda, Hb);
...
title(ax, '(a)','Interpreter','latex','FontSize',30);
box on
exportgraphics(fig,'SSE_XandP_wigner_panel_a_Ne_-0p75.pdf','ContentType', 'image', 'Resolution', 600);
...
title(ax, '(b)','Interpreter','latex','FontSize',30);
box on
exportgraphics(fig,'SSE_XandP_wigner_panel_b_Ne_0.pdf','ContentType', 'image', 'Resolution', 600);
...
title(ax, '(c)','Interpreter','latex','FontSize',30);
box on
exportgraphics(fig,'SSE_XandP_wigner_panel_c_Ne_+0p75.pdf','ContentType', 'image', 'Resolution', 600);
...
% box on
% exportgraphics(fig,'SSE_XandP_wigner_panel2_a_Ne_-0p75.pdf','ContentType', 'image', 'Resolution', 600);
% 
% % ---------- (b) N_e = 0 ----------
% fig = figure('Position', [100 100 800 600]);  % wider figure for visibility
% ax = axes(fig); hold(ax,'on');
...
% box on
% exportgraphics(fig,'SSE_XandP_wigner_panel2_b_Ne_0.pdf','ContentType', 'image', 'Resolution', 600);
% 
% % ---------- (c) N_e = +1.5 ----------
% fig = figure('Position', [100 100 800 600]);  % wider figure for visibility
% ax = axes(fig); hold(ax,'on');
...
% box on
% exportgraphics(fig,'SSE_XandP_wigner_panel2_c_Ne_+0p75.pdf','ContentType', 'image', 'Resolution', 600);
% 
...
title( sprintf(...
    'Lindblad  $(\\hat L_{\\phi})$ and $(\\hat L_{\\pi_{\\phi}})$, $\\lambda = %.2f$, $\\tilde \\mu = %.2f$', ...
    lambda, mu/Hb), ...
    'FontSize', 20, 'Interpreter', 'latex' );
% title( sprintf(...
%     'Lindblad , $H = %.2f$', Hb), ...
...
% Save video
LindVideoPath = sprintf('Wigner_XandP_LindVideo_lambda_%.2g_Hubble_%.2g.avi', lambda, Hb);
...
title(ax, '(a)','Interpreter','latex','FontSize',30);
box on
exportgraphics(fig,'Lindwigner_XandP_panel_a_Ne_-0p75.pdf','ContentType', 'image', 'Resolution', 600);
...
title(ax, '(b)','Interpreter','latex','FontSize',30);
box on
exportgraphics(fig,'Lindwigner_XandP_panel_b_Ne_0.pdf','ContentType', 'image', 'Resolution', 600);
...
title(ax, '(c)','Interpreter','latex','FontSize',30);
box on
exportgraphics(fig,'Lindwigner_XandP_panel_c_Ne_+0p75.pdf','ContentType', 'image', 'Resolution', 600);
% 
% 
% %
% 
...
SSEMP4Out  =sprintf( 'SSE_XandP_EigenbasisSimple_lambda_%.2g_Hubble_%.2g.mp4', lambda, Hb);
LindMP4Out = sprintf('Lind_XandP_EigenbasisSimple_lambda_%.2g_Hubble_%.2g.mp4', lambda, Hb);
