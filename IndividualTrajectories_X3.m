...
lambda    =0.005;      % Coupling strength
...
plotSpanInst=linspace(-2,-N0,1001);
...
tic
[~,PsiSSE, RhoSSE] = SSEDynamics_X3_Sparse(   bSize, hbar, mu, beta3, beta4, lambda, Hb,plotSpanSSE,dN_plot);
...
title( sprintf(...
    'SSE $(\\hat L_{\\phi^3})$, $\\lambda = %.2f$, $\\tilde \\mu = %.2f$', ...
    lambda, mu/Hb), ...
...
% Save video
SSEVideoPath = sprintf('Wigner_X3_SSEVideo_lambda_%.2g_Hubble_%.2g.avi', lambda, Hb);
...
[plotSpanLind, RhoLind] = Markov_LindbladX3_ExpStep_1000 ( TSpan,bSize, hbar, mu,beta3, beta4, lambda, Hb,true);
...
% expXLind=expXHam;
% expPLind=expPHam;
filenameWorkspace = sprintf('Workspace_X3_Individuals_lambda%.2g_Hubble_%.2g.mat', lambda, Hb);
...
box on
exportgraphics(fig,'SSE_X3_wigner_panel_a_Ne_-0p75.pdf','ContentType', 'image', 'Resolution', 600);
...
box on
exportgraphics(fig,'SSE_X3_wigner_panel_b_Ne_0.pdf','ContentType', 'image', 'Resolution', 600);
...
box on
exportgraphics(fig,'SSE_X3_wigner_panel_c_Ne_+0p75.pdf','ContentType', 'image', 'Resolution', 600);
...
title( sprintf(...
    'Lindblad  $(\\hat L_{\\phi^3})$, $\\lambda = %.2f$, $\\tilde \\mu = %.2f$', ...
    lambda, mu/Hb), ...
    'FontSize', 20, 'Interpreter', 'latex' );
% title( sprintf(...
%     'Lindblad , $H = %.2f$', Hb), ...
...
% Save video
LindVideoPath = sprintf('Wigner_X3_LindVideo_lambda_%.2g_Hubble_%.2g.avi', lambda, Hb);
...
title(ax, '(a)','Interpreter','latex','FontSize',30);
box on
exportgraphics(fig,'Lindwigner_X3_panel_a_Ne_-0p75.pdf','ContentType', 'image', 'Resolution', 600);
...
title(ax, '(b)','Interpreter','latex','FontSize',30);
box on
exportgraphics(fig,'Lindwigner_X3_panel_b_Ne_0.pdf','ContentType', 'image', 'Resolution', 600);
...
title(ax, '(c)','Interpreter','latex','FontSize',30);
box on
exportgraphics(fig,'Lindwigner_X3_panel_c_Ne_+0p75.pdf','ContentType', 'image', 'Resolution', 600);
% 
% 
% %
% 
...
varsToKeep = { ...
    ...
    'PsiSSE2','RhoSSE2', ...
    'RhoLind', ...
    'plotSpanInst','plotSpanHam','plotSpanSSE','plotSpanLind','N0','Nend','H0'};

clearvars('-except', varsToKeep{:});   % nukes x/p grids, HVector, etc.
...
SSEMP4Out  =sprintf( 'SSE_X3_EigenbasisSimple_lambda_%.2g_Hubble_%.2g.mp4', lambda, Hb);
LindMP4Out = sprintf('Lind_X3_EigenbasisSimple_lambda_%.2g_Hubble_%.2g.mp4', lambda, Hb);
