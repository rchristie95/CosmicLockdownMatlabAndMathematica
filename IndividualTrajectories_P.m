...
% ---------- Nonlinear contour levels (denser near low H) ----------

%% Single Lindblad

tic

[plotSpanLind, RhoLind] = Markov_LindbladP_ExpStep_1000 ( TSpan,bSize, hbar, mu,beta3, beta4, Hb,true);
...
% expXLind=expXHam;
% expPLind=expPHam;
filenameWorkspace = sprintf('Workspace_P_Individuals_Hubble_%.2g.mat', Hb);
...
[dN_plot, plotSpanSSE] = dnPPlot(N0, Nend,1e-5,1001);
% eta=(randn(Nsteps+1,1));
tic
[~,PsiSSE, RhoSSE] = SSEDynamics_P_ExpIntSparse(   bSize, hbar, mu, beta3, beta4, Hb,plotSpanSSE,dN_plot);
...
Z_final = arrayfun(@(x, y) Hinit([x; y], nFramesSSE), xMesh, pMesh);
...
title( sprintf(...
    'SSE $(\\hat L_{\\pi_{\\phi}})$, $\\tilde \\mu = %.2f$', mu/Hb), ...
...
% Save video
SSEVideoPath = sprintf('Wigner_P_SSEVideo_Hubble_%.2g.avi', Hb);
...
box on
exportgraphics(fig,'SSE_P_wigner_panel_P_a_Ne_-0p75.pdf','ContentType', 'image', 'Resolution', 600);
...
box on
exportgraphics(fig,'SSE_P_wigner_panel_P_b_Ne_0.pdf','ContentType', 'image', 'Resolution', 600);
...
box on
exportgraphics(fig,'SSE_P_wigner_panel_P_c_Ne_+0p75.pdf','ContentType', 'image', 'Resolution', 600);
...
title( sprintf(...
    'Lindblad  $(\\hat L_{\\pi_{\\phi}})$, $\\tilde \\mu = %.2f$',  mu/Hb), ...
    'FontSize', 20, 'Interpreter', 'latex' );
% title( sprintf(...
%     'Lindblad , $H = %.2f$', Hb), ...
%     'FontSize', 20, 'Interpreter', 'latex' );
...
% Save video
LindVideoPath = sprintf('Wigner_P_LindVideo_Hubble_%.2g.avi', Hb);
...
title(ax, '(a)','Interpreter','latex','FontSize',30);
box on
exportgraphics(fig,'Lindwigner_panel_P_a_Ne_-0p75.pdf','ContentType', 'image', 'Resolution', 600);
...
title(ax, '(b)','Interpreter','latex','FontSize',30);
box on
exportgraphics(fig,'Lindwigner_panel_P_b_Ne_0.pdf','ContentType', 'image', 'Resolution', 600);
...
title(ax, '(c)','Interpreter','latex','FontSize',30);
box on
exportgraphics(fig,'Lindwigner_panel_P_c_Ne_+0p75.pdf','ContentType', 'image', 'Resolution', 600);
% 
% 
% %
% 
...
[SSEAudioPath,  SSEAudioDur]  = write_simple_sonification('SSE_P',  RhoSSE,         plotSpanSSE,  Xhat, Phat, Hb, mu, beta3,  beta4,BSound, f0, fs);
[LindAudioPath, LindAudioDur] = write_simple_sonification('Lind_P', RhoLind,        plotSpanLind, Xhat, Phat, Hb, mu, beta3, beta4,BSound, f0, fs);
% % (Each *_AudioDur will be 30.000 s)

% ===================== Merge each with its 30s Wigner video =====================


SSEMP4Out  =sprintf( 'SSE_P_EigenbasisSimple_Hubble_%.2g.mp4', Hb);
LindMP4Out = sprintf('Lind_P_EigenbasisSimple_Hubble_%.2g.mp4', Hb);
