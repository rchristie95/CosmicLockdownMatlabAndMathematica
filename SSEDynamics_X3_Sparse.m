function [tvals, PsiOut, RhoOut] = SSEDynamics_X3_Sparse( ...
...
for s = 1:KnotN-1
    ...
    while t < t_next - tol
        ...
        [H0, L1_0] = opPackBig(t,      Xpow3b, Phat2b, lambda, mu, Vchi_b, Hb);
        % [H1, L1_1] = opPackBig(t+dt,   Xpow3b, Phat2b, lambda, mu, Vchi_b, Hb);
        if t_next>-1
...
%% ====================================================================
function [H, L1] = opPackBig(Ncur, Xpow3b, Phat2_b, lambda, mu, Vchi_b, Hb)
    % X-channel rate
    % Gamma = (131*pi*lambda^2 * exp(6*Ncur)) / (512*mu^5);
    Gamma = (pi*lambda^2 * exp(6*Ncur)) / (8*mu^3);
